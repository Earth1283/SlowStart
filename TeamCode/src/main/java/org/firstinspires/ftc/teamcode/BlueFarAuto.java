...
import org.firstinspires.ftc.teamcode.mechanisms.gate.ServoGate;
 ...
 *
 * Generated by the refract FTC suite (ftc-construct) from the team's confirmed
 * team-config.yaml: mecanum drivetrain, goBILDA Pinpoint odometry, Pedro Pathing,
 * raw SDK OpMode style, roller intake + flywheel shooter + multi-axis turret + gate.
 *
 * PATH: the eight segments the team supplied, split into six PathChains so the
 * robot can STOP and shoot. A single chain (the original MainChain) runs end to
 * end without stopping, which stop-and-shoot cannot use -- Pedro's own example
 * splits per scoring stop for the same reason. Shots fire after segments 1, 4
 * and 7, as specified.
 *
 *   toShoot   seg 1        start -> shootPose        then SHOOT (preload)
 *   pickup1   segs 2 + 3   shootPose -> pickup1Pose  intake running
 *   toShoot1  seg 4        pickup1Pose -> shootPose  then SHOOT
 *   pickup2   segs 5 + 6   shootPose -> pickup2Pose  intake running
 *   toShoot2  seg 7        pickup2Pose -> shootPose  then SHOOT
 *   park      seg 8        shootPose -> parkPose
 *
 * The turret is parked at a fixed setpoint for the whole run, per the team's choice.
 *
 * BEFORE THE FIRST RUN, none of these are optional:
 *   1. Hardware names in RobotConstants are PLACEHOLDERS -- init() throws if wrong.
 *   2. SHOOTER_TARGET_RPM is 0, so atTargetVelocity() is never true and the auto
 *      will WAIT AT THE SHOOT POSE FOREVER by design. Set a real RPM.
 *   3. GATE_OPEN/CLOSED_POSITION are both 0 -- the gate will not move.
 *   4. Verify motor directions and encoder directions per pedroPathing/Constants.java.
 */
...
public class BlueFarAuto extends OpMode {
    ...
    @Override
    public void start() {
        turret.setSetpoint(RobotConstants.TURRET_PARK_YAW_TICKS,
                           RobotConstants.TURRET_PARK_PITCH_TICKS);
        setPathState(0);
    }
    ...
    public void loop() {
        ...
        panelsTelemetry.debug("Shooter RPM", shooter.getCurrentVelocity());
    ...
    private void autonomousPathUpdate() {
        switch (pathState) {
            ...
            case 0:  // drive to the shooting pose, spinning the flywheel up on the way
                shooter.setTargetVelocity(RobotConstants.SHOOTER_TARGET_RPM);
