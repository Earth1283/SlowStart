...
import static org.firstinspires.ftc.teamcode.kernel.constants.RobotConstants.RED_TARGET_X;
import static org.firstinspires.ftc.teamcode.kernel.constants.RobotConstants.RED_TARGET_Y;
...
/**
 * Team 32008's own TeleOp, copied byte-for-byte from FTC-32008 V2 tests/A_1_AA_AS.java.
 *
 * RED alliance, driver-1 buttons, velocity lead ON, shooterHold when off.
 *
 * ONLY changes: package, the constants import path, and a name/group on @TeleOp so
 * these are findable on the picker. Not one line of logic is touched -- dead locals,
 * commented-out experiments and all. Diff this against V2 and expect zero drift.
 *
 * Frame is PINPOINT throughout. See teleop/AllianceTeleOp.java for the maintained
 * variant of the same OpMode.
 */
@TeleOp(name = "V2 A_1_AA_AS", group = "32008 V2")
@Configurable
...
//                if (timer.milliseconds() > 180) hoodCorrection = 0.045;
            ...
            if (gamepad1.dpadUpWasPressed() || gamepad2.dpadUpWasPressed()) distanceCorrection += 2;
            if (gamepad1.dpadDownWasPressed() || gamepad2.dpadDownWasPressed()) distanceCorrection -= 2;

            if (gamepad1.dpadLeftWasPressed() || gamepad2.dpadLeftWasPressed()) turretCorrection += 1;
            if (gamepad1.dpadRightWasPressed() || gamepad2.dpadRightWasPressed()) turretCorrection -= 1;
